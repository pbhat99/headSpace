...
def moveMenu(sourceMenuNames, destinationMenuName, icon, sourceRoot, destinationRoot, remove, contentsOnly):
    '''
    ...
    if destinationMenuName:
        ...
        allCommands = [menu.name() for menu in destinationRoot.items() if not isinstance(menu, nuke.Menu)]
...
def moveMenus(sourceMenus, destinationMenu = '', icon = 'noIconDefined', sourceRoot = 'Nodes', destinationRoot = '', remove = True, contentsOnly = False):
    '''
    ...
    QtCore.QTimer.singleShot(0, lambda: moveMenu(sourceMenus, destinationMenu, icon, sourceRoot, destinationRoot, remove, contentsOnly))