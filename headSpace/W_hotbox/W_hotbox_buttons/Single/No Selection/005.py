...
# v1.0
# fixed for windows file path (prasannakumar T Bhat) 05th march 2026
import os, re, nuke, inspect, sys
...
def imgSeq2NukeFormat(filesFound):
    for ff in filesFound:
      ...
      if padNum.isdigit():
          ...
          name='.'.join(splitFiles[:-2])
      else:
          padding=''
          name='.'.join(splitFiles[:-1])
      ext=splitFiles[-1]
      
      if padding:
          readFile=f"{name}.{padding}.{ext} {start}-{end}"
      else:
          readFile=f"{name}.{ext}"
      
      # Ensure path separators are clean
...
def getPath4Walk(walkPath=''):
      ...
      if verbose: print(f'failed here {lineno()}\n', file=sys.stderr)  
      return directory
